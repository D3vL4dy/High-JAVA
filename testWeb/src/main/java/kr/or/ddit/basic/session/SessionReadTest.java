...
public class SessionReadTest extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		out.println("<title>Session 데이터 확인</title></head>");
		out.println("<body>");
		...
		int cnt = 0; // 세션의 개수 확인용 변수
		while(sessionNames.hasMoreElements()) { // hasMoreElements() : 다음 데이터가 있는지 확인
			cnt++;
			
			// 각각 세션의 key값 구하기
			String sessionKey = sessionNames.nextElement();
			
			Object value = session.getAttribute(sessionKey); // 여러 종류의 데이터가 있기 때문에 Object로 
