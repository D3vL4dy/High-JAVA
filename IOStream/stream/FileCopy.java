...
//		File file = new File("d:/d_other/펭귄.jpg"); // 원본 데이터
		File file = fc.getSelecteFile("OPEN");
		if(file == null) {
		...
		File targetFile = fc.getSelecteFile("SAVE");
		if(targetFile == null) {
...
//			FileOutputStream fout = new FileOutputStream("d:/d_other/연습용/펭귄_복사본.jpg");
			...
			/*
			 * while((data = fin.read()) != -1) { fout.write(data); } fout.flush(); // 버퍼의
			 * 모든 내용을 출력소스에 출력 후 버퍼를 비운다.
			 * 
			 * System.out.println("복사 작업 끝...");
			 * 
	...
	public File getSelecteFile(String option) {
