...
public class ThreadTest02 {
	...
	public static void main(String[] args) {
		...
		th1.start(); // start()는 run()을 호출해주고 작업이 끝난다.
		
		
		// 2) Runnable인터페이스를 구현해서 사용하는 방법
		// ==> Runnable인터페이스를 구현한 class를 작성하고 이 class의 인스턴스를 생성한다. 
		//     그리고 Thread객체를 생성할 때 Thread의 생성자에 
		//     Runnable인터페이스를 구현한 클래스의 인스턴스 값을 넣어서 생성한 후 
		//     이 Thread객체의 start()메서드를 호출해서 실행한다. 
		
		MyRunner r1 = new MyRunner(); // 구현class의 인스턴스(객체) 생성
		...
		Runnable r2 = new Runnable() { // 일회성으로 사용하고 싶을 때
			@Override
			public void run() {
				for (int i = 1; i <= 200; i++) {
					System.out.print("@");
	...
	public void run() { // 쓰레드의 메인메서드
		...
		for (int i = 1; i <= 200; i++) {
			...
			try {
				...
				// '시간'은 밀리세컨드 단위를 사용한다. (1000은 1초를 의미)
			} catch (InterruptedException e) {}
		}
	}
}
