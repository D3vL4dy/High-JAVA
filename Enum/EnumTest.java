...
/*
 * - enum(열거형) 
 * 		==> 서로 관련있는 상수들의 집합을 나타낸다. (몇 가지로 제한된 상수를 가지는 타입) 
 * 		==> 클래스처럼 보이게 하는 상수
 ...
 *   3) valueOd("열거형상수명') ==> 저장된 열거형에서 '열거형상수명'과 일치하는 열거형 상수를 반환한다.
...
//		}
		...
		System.out.println("mycol : " + mycol.name()); // GREEN
		System.out.println("mycnt : " + mycnt.name()); // THREE
		
		System.out.println("mycol의 ordinal : " + mycol.ordinal()); // 1
		System.out.println("mycnt의 ordinal : " + mycnt.ordinal()); // 2
		System.out.println("----------------------------");
...
//		}
		...
		for(Season time : Season.values()) { //.values() : 열거형이 모든 상수들을 배열에 담아줌
